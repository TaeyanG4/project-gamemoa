...
#### 3.6.4 게임 관리 목록 — 활성화/비활성화 (2026-08-18)

`GET /api/admin/sandbox-games`가 삭제되지 않은 모든 게임을(개발자·공개 여부 무관) 반환합니다.
`/admin/sandbox-games` 페이지의 "게임 관리" 섹션이 이 목록을 렌더링해, 게임 ID를 미리 알지 못해도
바로 활성화(공개)/비활성화(비공개) 토글이 가능합니다 — 기존의 "게임 ID를 직접 입력해 검색"
흐름은 상세 관리(메타데이터 수정, 버전 이력, 삭제)용으로 그대로 남아 있습니다. 실제 토글 동작은
기존 `PATCH /api/admin/sandbox-games/:id/visibility`를 그대로 재사용합니다 — 새 권한이나 새 상태
축이 아니라, 이미 있던 기능을 찾기 쉽게 만든 UI 변경입니다.

#### 3.6.5 승인 철회 — 관리자가 승인 결정을 되돌리기 (2026-08-18)

`POST /api/admin/sandbox-games/versions/:versionId/revoke`는 이미 APPROVED된 버전의 결정을
**PENDING_REVIEW로 되돌립니다** — "관리자가 승인 결정 자체를 취소(재심사 대기로 되돌림)"이라는
2026-08-18 제품 결정에 따른 기능으로, 심사 전 게임 자체를 지우는 §3.6.3의 삭제와는 다릅니다.
`sandbox_games.review` 권한이면 충분합니다(별도 권한 없음, `decideVersion`과 동일 게이트).

- 대상 버전이 그 게임의 현재 라이브 버전이었다면, 같은 호출 안에서 `live_version_id`를 지우고
  `visibility`를 강제로 `PRIVATE`로 되돌립니다(소프트 삭제와 동일한 CHECK 제약 이유). 이미 다른
  버전이 라이브로 바뀐 상태라면 게임 자체는 건드리지 않고 대상 버전의 상태만 바뀝니다.
- 심사 슬롯(`review_slot`)은 **다시 점유하지 않습니다** — 슬롯은 "개발자가 새로 제출을 얼마나
  열어둘 수 있는가"를 제한하는 장치이지, 관리자가 자기 실수를 바로잡는 것까지 그 한도에 포함시킬
