...
**결론 및 단계별 계획**: 지금 당장 구현하지 않습니다. `GameManifest.supportsReplay`(기본
`false`) 필드만 먼저 마련해 두고, 다음 조건 중 하나가 충족되면 재검토합니다 — (1) 유저 게임
제작·등록 시스템(#16)이 실제로 진행되어 안티치트 필요성이 커질 때, 또는 (2) 리플레이 자체에
대한 실제 사용자 수요가 확인될 때. 착수 시 순서는: 게임 1개를 시드 PRNG로 전환 → 리플레이
기록/재생 프로토타입 검증 → 나머지 게임 확장.
