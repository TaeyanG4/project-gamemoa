...
## 남은 다국어 하드코딩 (확인됨, 아직 미착수)

- `game-slug.tsx`(실제 게임 플레이 화면)의 헤더 타이틀은 이번에 로케일화했지만, 같은 파일의
  나머지 UI 문구("게임을 찾을 수 없습니다", "목록으로 돌아가기", "게임 종료!", 결과 오버레이 전체
  등)는 여전히 하드코딩 한국어입니다 — 별도 작업 필요.
