# GAMEMOA Discord 앱 사용 및 운영 가이드

이 문서는 GAMEMOA Discord 연동을 처음 사용하는 서버 관리자, 일반 사용자, 운영자와 개발자를 위한 실무 가이드입니다. 현재 구현의 기준은 `apps/api/src/infrastructure/discord/commands.ts`, `interactionHandlers.ts`, `docs/DISCORD_INTEGRATION.md`입니다.

## 1. 먼저 알아둘 점

GAMEMOA v1은 Discord **HTTP Interactions**를 사용합니다.
...
Discord 앱 설치와 GAMEMOA 웹의 공개 서버 등록은 서로 다른 단계입니다.

> Discord 앱 설치 ≠ GAMEMOA 공개 서버 등록

앱을 설치했다고 서버가 자동으로 GAMEMOA 디렉토리에 게시되지 않습니다. 서버 관리자가 웹에서 공식 Discord 권한을 확인한 뒤 서버를 명시적으로 등록하고 가시성을 선택해야 합니다.

## 2. 서버 관리자: 설치와 서버 등록

### 2-1. Discord 앱 설치

GAMEMOA의 설치 링크는 Discord Developer Portal의 실제 설정을 기준으로 해야 합니다. 저장소에 애플리케이션 ID와 권한 비트가 없는 환경에서는 임의의 OAuth URL을 만들지 않으며, 웹 가이드에 설치 안내만 표시합니다.
...
### 2-2. GAMEMOA 웹에서 서버 등록

1. Discord 앱을 서버에 설치하거나 서버에서 사용할 수 있는 상태로 만듭니다.
2. Discord 계정이 필요한 경우 `/gamemoa link`를 실행합니다.
3. GAMEMOA 웹에 로그인합니다.
4. `/discord/servers`에서 `내 서버 등록하기`를 선택합니다.
5. GAMEMOA가 시작한 Discord OAuth에서 `identify guilds` 권한을 승인합니다.
6. GAMEMOA는 Discord 공식 `/users/@me/guilds` 응답에서 본인이 소유자이거나 `MANAGE_GUILD` 또는 `ADMINISTRATOR` 권한을 가진 길드만 후보로 보여줍니다.
...
1. Discord 서버 채널에서 `/gamemoa link`를 실행합니다.
...
| ---------------------- | -------------------------------------------- | ---------------------------------- | ------------- | --------- | ---------------------- | ------------------------ |
| `/gamemoa games`       | 현재 게시된 게임 목록과 링크 확인            | 명령어가 표시되는 Discord 컨텍스트 | 불필요        | 불필요    | `/gamemoa games`       | `/games`                 |
| `/gamemoa link`        | Discord 계정과 GAMEMOA 계정 연결 시작        | 명령어가 표시되는 Discord 컨텍스트 | 미연결 사용자 | 불필요    | `/gamemoa link`        | `/discord/link`          |
| `/gamemoa profile`     | 연결된 프로필의 닉네임, 레벨, 글로벌 XP 확인 | 명령어가 표시되는 Discord 컨텍스트 | 필요          | 불필요    | `/gamemoa profile`     | `/profile`               |
| `/gamemoa play`        | 서버 귀속 플레이 링크 생성                   | Discord 서버 채널                  | 필요          | 필요      | 게임 선택은 선택 사항  | `/games`                 |
| `/gamemoa rank`        | 현재 서버에서 자신의 XP와 순위 확인          | Discord 서버 채널                  | 필요          | 필요      | `/gamemoa rank`        | 해당 서버 페이지         |
| `/gamemoa leaderboard` | 서버 XP Top 10과 전체 페이지 링크 확인       | Discord 서버 채널                  | 불필요        | 필요      | `/gamemoa leaderboard` | `/discord/servers/:slug` |
| `/gamemoa server`      | 서버 전체 XP, 주간 XP, 참여자 수 확인        | Discord 서버 채널                  | 불필요        | 필요      | `/gamemoa server`      | `/discord/servers/:slug` |
...
```text
/gamemoa play
/gamemoa play game:reaction-time
```
...
- 계정 연결 필요: `/gamemoa link`를 먼저 실행합니다.
...
- 링크 만료 또는 재사용: 새 `/gamemoa play` 링크를 발급합니다.

## 4. XP가 계산되는 방식

GAMEMOA에는 서로 다른 세 가지 XP가 있습니다.

1. **일반 GAMEMOA XP**: 사용자 글로벌 진행도와 레벨에 사용됩니다.
...
Guild A에서 /gamemoa play로 시작한 유효한 게임 완료 +10:
글로벌 XP = 25,010
Guild A 사용자 XP = 10
Guild A 활동 XP = +10
Guild B = 0
...
`/gamemoa play` 링크로 시작한 완료만 해당 Guild에 귀속됩니다. 서버 귀속은 1회용 Play Context와 XP 원장의 source ID를 함께 확인하며, 하나의 글로벌 XP 원장 이벤트는 최대 하나의 Guild 귀속만 가질 수 있습니다.

## 5. 서버 랭킹과 게임 기록

- **서버 XP 랭킹**: 해당 Guild의 서버별 사용자 XP 합계입니다.
- **주간 서버 XP**: 월요일 00:00 Asia/Seoul 기준으로 계산하며 UTC로 저장된 시각을 변환해 집계합니다.
- **게임별 서버 기록**: 해당 서버에서 GAMEMOA 활동을 만든 참여자의 기존 `scores` 최고 기록을 게임 매니페스트 정렬 정책으로 표시합니다.
...
- 참여자 수는 GAMEMOA XP를 실제로 귀속한 사용자 수이며, GAMEMOA가 Discord의 전체 멤버 목록을 열거한다는 뜻이 아닙니다.
...
1. Discord Developer Portal에서 GAMEMOA Application을 엽니다.
...
- `pnpm discord:commands:register`: 명령어 등록 후 Discord에서 `/gamemoa` 선택 목록 확인
...
앱 설치 컨텍스트, `applications.commands` 설정, 명령어 등록 성공 여부, 전역 명령어 전파 지연을 순서대로 확인합니다. 앱을 다시 설치하는 것만으로 GAMEMOA 서버 등록이 완료되지는 않습니다.

### 서버 등록 후보가 없음

로그인한 Discord 사용자가 길드 소유자이거나 `MANAGE_GUILD` 또는 `ADMINISTRATOR` 권한을 갖는지 확인합니다. GAMEMOA는 모든 Discord 멤버를 나열하지 않으며 공식 OAuth `guilds` 응답만 사용합니다.

### XP가 서버에 쌓이지 않음

서버가 `ACTIVE`인지, Discord 계정과 GAMEMOA 계정이 연결되었는지, `/gamemoa play` 링크가 만료되지 않았는지, 링크를 통해 실제 게임 완료를 제출했는지 확인합니다. 글로벌 XP 일일 상한에 도달하면 서버 XP 증가액도 0일 수 있습니다.
