...
1. **업로드**: [게임 크리에이터 센터](/game-creator)에서 ZIP 최상위에 `owogg.game.json`
   (slug/title/genre 포함)을 넣어 둔 번들을 페이지에 끌어다 놓습니다 — 그 한 번으로 게임 등록과
   첫 버전 업로드가 끝납니다. 형식은 [`GAME_CREATION_GUIDE.md`](GAME_CREATION_GUIDE.md) §3.6.2
   참고. 업로드 직후는 본인에게만 보이며, 다른 유저에게는 어디에도 노출되지 않습니다. 이미 등록한
   게임에 새 버전을 올릴 때는 목록의 "버전 업로드"를 씁니다(이때는 `owogg.game.json`이 필요
   없습니다).
...
경우 즉시 비공개로 전환되고 게임 기록은 감사 목적으로 남습니다 — 필요하면 관리자에게 문의하세요.
