...
export default function AdminCreatorsRoute() {
  ...
  const handleAction = async (jobId: number, action: CreatorManualReviewAction) => {
    ...
    setError(null);
    try {
      await applyManualCreatorReviewApi(jobId, { action, reason });
      setReasons((previous) =>
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 md:px-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-text-primary">Creator 수동 심사</h1>
                ...
                return (
                  ...
                  >
                    ...
                    <dl className="mt-4 grid grid-cols-2 gap-3 text-xs md:grid-cols-4">
                      <DataPoint
                        label="소유권"
                        value={`${item.creatorStatus} / ${item.platformAccount.verificationStatus}`}
                      />
                      <DataPoint
                        label="구독자/팔로워"
                        value={
