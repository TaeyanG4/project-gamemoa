...
export function meta() {
  return [
    { title: "문제 해결 | GAMEMOA Wiki" },
    { name: "description", content: "GAMEMOA Discord 연동 문제 해결 가이드" },
  ];
}
  ...
  return (
    ...
    >
      <WikiCallout tone="warning">
        ...
        받았다면 공식 GAMEMOA 채널이 아닐 수 있습니다.
      </WikiCallout>

      <div className="space-y-3">
        <WikiFaqItem question="/gamemoa가 자동완성에 안 나옵니다">
          Discord 클라이언트를 재시작하거나 서버를 나갔다가 다시 들어와 보세요. 그래도 안 나오면
          앱이 이 서버에 실제로 설치되어 있는지 서버 관리자에게 확인을 요청하세요. GAMEMOA 운영진
          쪽에서는{" "}
        ...
        <WikiFaqItem question="/gamemoa link를 입력했는데 일반 메시지로 올라갑니다">
          정상적인 슬래시 명령어 대신 일반 텍스트로 전송됐다면 Discord가 명령어를 인식하지 못한
          것입니다. 자동완성 목록에서 정확히 <code>/gamemoa</code>를 선택한 뒤 하위 명령어를
          선택해서 실행해야 합니다. 직접 타이핑해서 전송하면 일반 메시지가 됩니다.
        </WikiFaqItem>

        <WikiFaqItem question="애플리케이션이 응답하지 않았습니다">
          일시적인 지연이나 오류일 수 있습니다. 잠시 후 다시 시도하세요. 반복되면 GAMEMOA 서비스
          상태에 문제가 있을 수 있으니 잠시 후 다시 확인해주세요.
        </WikiFaqItem>

        <WikiFaqItem question="계정이 이미 연결되어 있다고 합니다">
          이 Discord 계정이 이미 다른 GAMEMOA 계정과 연결되어 있다는 뜻입니다. 하나의 Discord 계정은
          하나의 GAMEMOA 계정에만 연결할 수 있습니다. 다른 계정에 연결하려면 먼저 웹에서 기존 연결을
          해제해야 합니다.
        </WikiFaqItem>

        <WikiFaqItem question="/gamemoa play가 서버 미등록이라고 합니다">
          이 Discord 서버가 아직 GAMEMOA 커뮤니티로 등록되지 않았습니다. 서버 관리자가{" "}
          <Link
        ...
        <WikiFaqItem question="봇이 Discord 멤버 목록에서 보이지 않습니다">
          GAMEMOA는 상시 접속(Gateway) 봇이 아니라 서명된 HTTP Interactions 방식으로 동작합니다.
