...
export function LoginModal() {
  ...
  const handleGoogleLogin = () => {
    ...
    loginWithGoogle();
    // Loading state will be cleared when the modal closes on success
    // or if an error occurs (context handles that)
    setTimeout(() => setIsGoogleLoading(false), 5000); // Fallback timeout
  };
  ...
  const handleDiscordLogin = () => {
    ...
    loginWithDiscord();
    // This navigates away, so no need to reset
  };
      ...
      <div className="relative w-full max-w-md bg-surface-raised border border-border rounded-3xl p-6 md:p-8 shadow-2xl flex flex-col gap-6 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-col gap-3.5 w-full">
          ...
          <button
            ...
            className="flex items-center justify-center gap-3 w-full py-4 px-4 bg-white hover:bg-slate-100 text-slate-900 font-extrabold rounded-2xl transition-all shadow-lg hover:scale-[1.02] cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
            {isGoogleLoading ? (
          ...
          <button
            ...
            disabled={isGoogleLoading || isDiscordLoading}
            className="flex items-center justify-center gap-3 w-full py-4 px-4 bg-[#5865F2] hover:bg-[#4752C4] text-white font-extrabold rounded-2xl transition-all shadow-lg hover:scale-[1.02] cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
            {isDiscordLoading ? (
