...
function createReviewDb() {
  ...
  return {
    prepare(sql: string) {
      ...
      if (sql.includes("creator_platform_accounts WHERE id =")) {
        return {
          bind() {
            return {
              async first() {
      ...
      if (sql.includes("creator_profiles WHERE id =")) {
        return {
          bind() {
            return {
              async first() {
      ...
      if (sql.includes("UPDATE creator_platform_accounts")) {
        return {
          bind() {
            return {
              async run() {
      ...
      if (sql.includes("UPDATE creator_review_jobs")) {
        return {
          bind() {
            return {
              async run() {
