...
import { YouTubeCreatorProvider } from "../src/infrastructure/creators/youtube.js";
import { ChzzkCreatorProvider } from "../src/infrastructure/creators/chzzk.js";
import { SoopCreatorProvider } from "../src/infrastructure/creators/soop.js";
...
test("YouTube: channel hiding subscriberCount leaves audienceCount undefined (UNKNOWN)", async () => {
  await withMockFetch(
    ...
    async () => {
      const provider = new YouTubeCreatorProvider("id", "secret", "apikey");
...
test("CHZZK: missing followerCount leaves audienceCount undefined (UNKNOWN)", async () => {
  await withMockFetch(
    ...
    async () => {
      const provider = new ChzzkCreatorProvider("id", "secret");
      const info = await provider.verifyOwnershipCode("code", "redirect");
      assert.equal(info.audienceCount, undefined);
    },
  );
...
test("SOOP: missing fan_count leaves audienceCount undefined (UNKNOWN)", async () => {
  await withMockFetch(
    ...
    async () => {
      const provider = new SoopCreatorProvider("id", "secret");
      const info = await provider.verifyOwnershipCode("code", "redirect");
      assert.equal(info.audienceCount, undefined);
    },
  );
