...
/**
 * typing-test's own thin wrapper over the shared standalone Bridge adapter
 ...
 *
 * typing-test has no difficulty tiers (manifest.ts has no `difficulty` field, and game.tsx never
 * reads `runtime.difficultyId`), so "normal" here is a pure placeholder — the same role
 * reaction-time's own standalone/bridgeRuntime.ts gives it.
 */
