 ...
 * version-hashed path (official-games/aim-test/<hash>/, see gameServing.ts). Runs vite's own JS
 * build API directly (rather than shelling out to `pnpm build`) so this test has no dependency on
 * turbo's task ordering — `test` does not depend on this package's own `build` task (see
...
test("standalone build produces a relative-path index.html plus JS/CSS bundle assets", async () => {
  ...
  // Relative, not absolute — this bundle is served from official-games/aim-test/<hash>/, never
  // the origin root, and the hash segment is only known at publish time (see
