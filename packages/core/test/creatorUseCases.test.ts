...
class MockCreatorRepo implements CreatorRepository {
  public profiles: Map<number, CreatorProfile> = new Map();
  ...
  async addPlatformAccount(input: {
    ...
    if (existing) {
      throw new Error(
        `Platform account ${input.platform}:${input.platformUserId} is already linked to another creator`,
      );
    }

    const now = new Date().toISOString();
    const verStatus = input.verificationStatus ?? "VERIFIED";
    const acc: CreatorPlatformAccount = {
      id: this.nextAccId++,
  ...
  async getCreatorRankings(options: {
    ...
    const verifiedCreators = Array.from(this.profiles.values()).filter(
      (p) => p.status === "VERIFIED",
    );

    const filtered = verifiedCreators.filter((p) => {
    ...
    if (options.mode === "score") {
      const candidates: CreatorRankEntry[] = [];
      ...
      for (const c of filtered) {
        ...
        const pAccs = this.platformAccounts
          .filter((a) => a.creatorId === c.id && a.verificationStatus === "VERIFIED")
          .map((a) => ({
            platform: a.platform,
...
test("Phase D Creator Domain & Ranking Invariants", async (t) => {
  let repo: MockCreatorRepo;
  let useCases: CreatorUseCases;

  t.beforeEach(() => {
    repo = new MockCreatorRepo();
    useCases = new CreatorUseCases(repo);

    // Register 3 users:
    // User 1: Normal player (no creator profile)
    ...
    // User 3: Verified Creator (YouTube & Twitch, Featured)
  ...
  await t.test("1. Normal user (User 1) is excluded from Creator rankings", async () => {
    const creator2 = await repo.upsertProfile({ userId: 2, status: "VERIFIED" });
    ...
    const res = await useCases.getCreatorRankings({ mode: "score", gameId: "aim-test" });
    assert.equal(res.total, 1);
    assert.equal(res.entries[0].userId, 2);
    // User 1 (9999 pts) is excluded because status !== 'VERIFIED'
  });

  await t.test("2. Verified creators are included and ranked by canonical score", async () => {
  ...
  await t.test("3. Platform filter strictly filters creator accounts", async () => {
    const c2 = await repo.upsertProfile({ userId: 2, status: "VERIFIED" });
    await repo.addPlatformAccount({
      creatorId: c2.id,
      platform: "CHZZK",
    ...
    // Filter YOUTUBE only
    const ytRes = await useCases.getCreatorRankings({ mode: "score", platform: "YOUTUBE" });
    assert.equal(ytRes.total, 1);
    assert.equal(ytRes.entries[0].userId, 3);

    // Filter CHZZK only
    const chzzkRes = await useCases.getCreatorRankings({ mode: "score", platform: "CHZZK" });
  ...
  await t.test("5. Featured or Partner status never modifies scores or XP", async () => {
    ...
    const scoreRes = await useCases.getCreatorRankings({ mode: "score" });
    assert.equal(scoreRes.entries[0].score, 8500); // Raw canonical score preserved

    const xpRes = await useCases.getCreatorRankings({ mode: "xp" });
    assert.equal(xpRes.entries[0].totalXp, 20000); // Raw global XP preserved
  });

  await t.test("6. Empty creator database returns total 0 cleanly", async () => {
    const res = await useCases.getCreatorRankings({ mode: "score" });
