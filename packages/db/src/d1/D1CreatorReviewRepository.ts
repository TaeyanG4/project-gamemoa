import type {
  ...
  CreatorManualReviewItem,
  CreatorPlatformType,
  CreatorReviewAction,
  CreatorReviewAuditLog,
  CreatorReviewAuditResult,
  CreatorReviewJob,
  CreatorReviewJobStatus,
  CreatorReviewMetricSnapshot,
...
function mapReviewJobRow(r: Record<string, unknown>): CreatorReviewJob {
  return {
    ...
    reviewType: String(r.review_type ?? "ACQUISITION") as CreatorReviewType,
...
const MANUAL_REVIEW_SELECT = `
  SELECT
    ...
    cp.id AS review_creator_id,
    cp.status AS review_creator_status,
    cp.featured_status AS review_featured_status,
    cpa.id AS review_account_id,
    cpa.creator_id AS review_account_creator_id,
  ...
  FROM creator_review_jobs rj
  JOIN creator_platform_accounts cpa ON cpa.id = rj.creator_platform_account_id
  JOIN creator_profiles cp ON cp.id = cpa.creator_id
  JOIN users u ON u.id = cp.user_id
`;

function mapManualReviewRow(row: Record<string, unknown>): CreatorManualReviewItem {
  return {
    job: mapReviewJobRow(row),
    userId: Number(row.review_user_id),
    nickname: String(row.review_nickname),
    creatorId: Number(row.review_creator_id),
    creatorStatus: String(row.review_creator_status) as CreatorManualReviewItem["creatorStatus"],
    featuredStatus: String(row.review_featured_status) as CreatorManualReviewItem["featuredStatus"],
    platformAccount: {
      id: Number(row.review_account_id),
      creatorId: Number(row.review_account_creator_id),
      platform: String(row.review_platform) as CreatorPlatformType,
...
function mapAuditRow(row: Record<string, unknown>): CreatorReviewAuditLog {
  let metricSnapshot: CreatorReviewMetricSnapshot | null = null;
  if (typeof row.metric_snapshot_json === "string") {
    try {
      const parsed = JSON.parse(row.metric_snapshot_json) as CreatorReviewMetricSnapshot;
  ...
  return {
    id: Number(row.id),
    creatorPlatformAccountId: Number(row.creator_platform_account_id),
    reviewJobId:
      row.creator_review_job_id === null || row.creator_review_job_id === undefined
        ? null
        : Number(row.creator_review_job_id),
    reviewerUserId: Number(row.reviewer_user_id),
    action: String(row.action) as CreatorReviewAction,
    reason: String(row.reason),
    previousStatus: String(row.previous_status) as CreatorReviewJobStatus,
    newStatus: String(row.new_status) as CreatorReviewJobStatus,
    metricSnapshot,
    createdAt: String(row.created_at),
    platform: row.audit_platform ? (String(row.audit_platform) as CreatorPlatformType) : undefined,
...
export class D1CreatorReviewRepository implements CreatorReviewRepository {
  ...
  async findActiveJobByAccountId(
    ...
    const row = await this.db
      .prepare(
        `SELECT * FROM creator_review_jobs
         WHERE creator_platform_account_id = ?
  ...
  async findLatestRevalidationJobByAccountId(
    ...
    const row = await this.db
      .prepare(
        `SELECT * FROM creator_review_jobs
         WHERE creator_platform_account_id = ? AND review_type = 'REVALIDATION'
         ORDER BY id DESC LIMIT 1`,
      )
      .bind(creatorPlatformAccountId)
      .first<Record<string, unknown>>();
  ...
  async createOrResetJob(input: {
    ...
    await this.db
      .prepare(
        `INSERT INTO creator_review_jobs
         (creator_platform_account_id, review_type, status, initial_audience,
    ...
    const created = await this.db
      .prepare(
        `SELECT * FROM creator_review_jobs
         WHERE creator_platform_account_id = ? AND review_type = 'ACQUISITION'
         ORDER BY id DESC LIMIT 1`,
      )
    ...
    await this.db
      .prepare(
        `INSERT INTO creator_review_jobs
         ...
         FROM creator_platform_accounts WHERE id = ?`,
      )
      .bind(input.nextCheckAt, input.nowIso, input.nowIso, input.creatorPlatformAccountId)
      .run();

    const created = await this.findLatestRevalidationJobByAccountId(input.creatorPlatformAccountId);
    if (created) return created;
    throw new Error("Failed to create creator revalidation job");
  }
      ...
      .prepare(
        `INSERT INTO creator_review_jobs
         ...
         WHERE cp.featured_status = 'FEATURED'
           ...
           AND NOT EXISTS (
             SELECT 1 FROM creator_review_jobs existing
             WHERE existing.creator_platform_account_id = cpa.id
  ...
  async listManualReviewQueue(limit: number, offset: number): Promise<CreatorReviewQueueResult> {
    const boundedLimit = Math.min(Math.max(limit, 1), 50);
    const boundedOffset = Math.max(offset, 0);
    const countRow = await this.db
      .prepare(`SELECT COUNT(*) AS total FROM creator_review_jobs WHERE status = 'MANUAL_REVIEW'`)
      .first<{ total: number }>();
  ...
  async listAuditLogs(limit: number, offset: number): Promise<CreatorReviewAuditResult> {
    const boundedLimit = Math.min(Math.max(limit, 1), 100);
    const boundedOffset = Math.max(offset, 0);
    const countRow = await this.db
      .prepare(`SELECT COUNT(*) AS total FROM creator_review_audit_log`)
      .first<{ total: number }>();
    const rows = await this.db
      .prepare(
        `SELECT audit.*, cpa.platform AS audit_platform, cpa.channel_name AS audit_channel_name
         FROM creator_review_audit_log audit
         LEFT JOIN creator_platform_accounts cpa
           ON cpa.id = audit.creator_platform_account_id
  ...
  private async findManualReviewItemByJobId(
    jobId: number,
  ): Promise<CreatorManualReviewItem | null> {
    const row = await this.db
      .prepare(
        `${MANUAL_REVIEW_SELECT}
         WHERE rj.id = ? LIMIT 1`,
  ...
  async applyManualReviewDecision(input: {
    ...
    if (
      ...
      (item.creatorStatus !== "VERIFIED" || item.platformAccount.verificationStatus !== "VERIFIED")
    ) {
      return {
        applied: false,
    ...
    if (input.action === "KEEP_FOR_REVIEW") {
      const duplicate = await this.db
        .prepare(
          `SELECT id FROM creator_review_audit_log
           WHERE creator_review_job_id = ? AND reviewer_user_id = ?
    ...
    const newStatus: CreatorReviewJobStatus =
      input.action === "APPROVE_FEATURED"
        ? "FEATURED"
    ...
    } satisfies CreatorReviewMetricSnapshot);
    ...
    statements.push(
      this.db
        .prepare(
          `INSERT INTO creator_review_audit_log
           (creator_platform_account_id, creator_review_job_id, reviewer_user_id,
          ...
          .prepare(
            `INSERT INTO creator_review_jobs
             ...
             WHERE id = ?
               AND NOT EXISTS (
                 SELECT 1 FROM creator_review_jobs existing
                 WHERE existing.creator_platform_account_id = ?
                   AND existing.review_type = 'REVALIDATION'
                   AND (
                     existing.status IN (${REVALIDATION_ACTIVE_STATUSES})
                     OR existing.status = 'MANUAL_REVIEW'
