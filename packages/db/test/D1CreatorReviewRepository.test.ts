...
import { D1CreatorReviewRepository } from "../src/d1/D1CreatorReviewRepository.js";
...
function createDb() {
  ...
  const manualRow = {
    ...
    review_creator_id: 1,
    review_creator_status: "VERIFIED",
    review_featured_status: "NONE",
    review_account_id: 11,
    review_account_creator_id: 1,
  ...
  const db = {
    prepare(query: string) {
      ...
      return {
        ...
        async first<T>() {
          if (query.includes("FROM creator_review_jobs rj")) return manualRow as T;
          return null;
        },
        async all<T>() {
          return { results: [] as T[] };
...
test("D1CreatorReviewRepository — manual approval writes an immutable audit insert and revalidation job", async () => {
  const { db, queries } = createDb();
  const repository = new D1CreatorReviewRepository(db as any);
  ...
  assert.ok(queries.some((query) => query.includes("INSERT INTO creator_review_audit_log")));
