...
function seedVerifiedCreator(
  ...
  raw
    ...
    .run(creatorId, platform, platformUserId, now, now);
  return creatorId;
}
...
test("Creator score ranking: one Creator's hundreds of scores never suppress others, and total is exact", async () => {
  ...
  seedVerifiedCreator(raw, userA, "YOUTUBE", "yt-a");
  seedVerifiedCreator(raw, userB, "TWITCH", "tw-b");
  seedVerifiedCreator(raw, userC, "YOUTUBE", "yt-c");
  ...
  const result = await repo.getCreatorRankings({
    mode: "score",
  ...
  assert.equal(result.total, 3, "total counts eligible Creator users, not raw rows");
...
test("Creator score ranking: platform filter uses EXISTS and does not duplicate rows", async () => {
  const { db, raw } = createSqliteD1(LEADERBOARD_TEST_SCHEMA);
  const userA = seedUser(raw, "A");
  seedVerifiedCreator(raw, userA, "YOUTUBE", "yt-a");
  // A second, unrelated verified platform account for a different creator so EXISTS has something to filter.
  const userB = seedUser(raw, "B");
  seedVerifiedCreator(raw, userB, "TWITCH", "tw-b");
...
test("Creator score ranking: pagination (offset/limit) applies to the deduplicated PB set", async () => {
  const { db, raw } = createSqliteD1(LEADERBOARD_TEST_SCHEMA);
  const ids: number[] = [];
  for (let i = 0; i < 5; i++) {
    const uid = seedUser(raw, `c${i}`);
    ids.push(uid);
    seedVerifiedCreator(raw, uid, "YOUTUBE", `yt-${i}`);
    for (let j = 0; j < 20; j++) {
  ...
  const repo = new D1CreatorRepository(db);
  const page1 = await repo.getCreatorRankings({
    mode: "score",
    gameId: "reaction-time",
    direction: "desc",
    limit: 2,
  ...
  const page2 = await repo.getCreatorRankings({
    mode: "score",
    gameId: "reaction-time",
    direction: "desc",
    limit: 2,
